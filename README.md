...
**WASD 操控鬼魂**移动到物品旁边 → **按空格附身** → **点击物品闹鬼**，制造各种灵异现象，把闯入者**吓得落荒而逃**。

- 👻 WASD 操控鬼魂 → 空格附身物品 → 点击闹鬼
- 🪑 闹鬼效果 → 物品抖动、闪烁、移动、发出怪声
- 😱 人类 AI → 疑惑 → 受惊 → 惊恐 → 逃跑，5 种状态逐级升级
- 🔥 连续点击 → 触发连击加成，伤害更高
- ⚡ 管理能量 → 能量有限，合理分配每次闹鬼
- ⭐ 速通评级 → 越快吓跑所有人，星级越高
- 🔊 全局音效 → 物品、人类、鬼魂均有对应音效
...
│   │
│   ├── ghost/                 # 鬼魂系统（核心玩法）
│   │   ├── GhostController.ts # 鬼魂控制器（WASD移动 + 空格附身 + 点击闹鬼）
│   │   ├── HauntableItem.ts   # 可附身物品（动画、冷却、惊吓值、附身状态）
│   │   ├── ItemClickHandler.ts # 物品点击处理（附身检查 → 转发闹鬼）
...
- [x] 核心鬼魂交互（WASD移动 + 附身 + 点击闹鬼）
- [x] 人类 AI（恐惧值/5 状态机/帧动画/逃跑）
- [x] 能量系统 + 连击系统
- [x] 关卡配置 + 计时 + 三星评价
- [x] 存档系统 + 金币 + 皮肤商店
- [x] 资产接入（客厅背景/小明小红 5 套动画/11 个音效）
- [ ] 10 个关卡内容填充
- [ ] 抖音 SDK 接入（分享、激励视频）
- [ ] 性能优化 + 真机测试
- [ ] TapTap 商店页 + 上线
